...
class picking_SlacAlgorithm:
    ...
    def step(self, env, ob, t, is_random):
        ...
        if is_random:
            ...
            action = np.append([0, 0], action)
        state, reward, done, _ = env.step(action)
        img = state[0][0]
        tactile = state[1]
        mask = False if t == env._max_episode_steps else done
        ob.append(img, tactile,  action[2:4])
        self.buffer.append(action[2:4], reward, mask, img, tactile, done)
        self.total_step += 1
    ...
    def update_latent(self, writer):
        ...
        loss_kld, loss_image, loss_reward, alignment_loss, contact_loss = self.latent.calculate_loss(state_, tactile_, action_, reward_, done_)
